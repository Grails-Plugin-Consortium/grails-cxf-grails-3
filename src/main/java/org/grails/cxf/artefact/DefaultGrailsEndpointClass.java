...
//	}
	...
	private String getConfiguredExpose(Object propExpose) {
		...
		if (propExpose instanceof EndpointType) {
			manualExpose = (propExpose).toString();
	...
	protected void setupServletName() {
		String manualServletName = getPropertyOrStaticPropertyOrFieldValue(PROP_SERVLET_NAME, String.class);
	...
	protected void findWsdl() {
		...
		if (wsdlLocation != null && !wsdlLocation.equals("")) {
			...
			if (wsdl == null) {
				log.error("Endpoint [" + getFullName() + "] as WSDL at [" + wsdlLocation
						+ "] but it couldn't be found. Will try to generate the Cxf Service from the endpoint class.");
			}
		}
